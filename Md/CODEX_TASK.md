...
`DATA\_SOURCES.md`에 실제 수와 차이를 기록한다.
...
```text
1. 세 슬롯 1.04배 확대 — 160ms
2. 하늘색/민트 테두리 Glow — 300ms
3. 작은 별 파티클 표시 — 500ms
4. 배경 오버레이 opacity 0.20
5. 카드가 아래 24px 위치에서 중앙으로 Pop
6. 카드 Y축 Flip
7. 카드 앞면 표시
```
...
```text
1. 슬롯 결과 확정
2. 화면 오버레이를 300ms 동안 어둡게
3. 금색 Glow 표시
4. 화면 컨테이너를 좌우 최대 4px로 240ms 흔들기
5. 중앙으로 금빛 입자 집결
6. 120ms Flash
7. 카드 Pop
8. 카드 Flip
...
```text
1. 슬롯 결과 확정
2. 화면 오버레이를 400ms 동안 어둡게
3. 라벤더/연핑크 입자를 중앙으로 이동
4. 카드 실루엣 표시
5. 카드 주변 원형 Glow 표시
6. 카드 Pop
7. 일반/전설보다 느린 Flip
8. MYTHICAL DISCOVERED 표시
```
...
실패에서는 당첨용 Flash, 카드 Pop, 파티클을 실행하지 않는다.
